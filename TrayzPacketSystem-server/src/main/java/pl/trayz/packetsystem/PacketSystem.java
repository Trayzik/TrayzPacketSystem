...
public class PacketSystem {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            hostname = args.length != 2 ? "127.0.0.1" : args[0];
        }catch (NumberFormatException e) {
            Logger.logError("Enter correct port!");
            System.exit(1);
            return;
        }
        if(args.length != 2) Logger.logWarning("If you want to use different port than the default, fill startup arguments (ip port)!");
        SERVER_THREAD.submit(() -> {
            ServerSocket server = null;
            try {
                server = new ServerSocket(port, 50, InetAddress.getByName(hostname));
